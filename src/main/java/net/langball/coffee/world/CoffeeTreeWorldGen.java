...
public class CoffeeTreeWorldGen implements IWorldGenerator {
	@Override
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator,IChunkProvider chunkProvider) {
	    ...
	    if (random.nextFloat() < ConfigLoader.coffee_tree_int / 8.0F)
	    {
	      int posX = x + world.rand.nextInt(16);
	      int posZ = z + world.rand.nextInt(16);
	      BlockPos newPos = getGroundPos(world, posX, posZ);
	      if ((newPos != null) && (BlockLoader.Coffee_tree.canPlaceBlockAt(world, newPos))) {
	        world.setBlockState(newPos, BlockLoader.Coffee_tree.getDefaultState(), 2);
	      }
	    }
	}
	@Nullable
